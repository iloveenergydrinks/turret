...
contract TurretRecoverableStakingSafetyTest is Test {
    ...
    function testSenderSurchargeCannotDebitMoreTURRETThanReviewed() public {
        ...
        vm.prank(alice); vm.expectRevert(TurretRecoverableStaking.UnsupportedTransfer.selector); staking.stake(10e18);
        assertEq(token.balanceOf(alice), 100e18);
    ...
    function testTaxedStakingTransferCannotCreateUnbackedStake() public {
        ...
        vm.prank(alice); vm.expectRevert(TurretRecoverableStaking.UnsupportedTransfer.selector); staking.stake(100e18);
    ...
    function testSenderSurchargeCannotConsumeExtraRewardFunding() public {
        ...
        vm.expectRevert(TurretRecoverableStaking.UnsupportedTransfer.selector); staking.distribute(5e6);
    ...
    function testTaxedOutgoingRewardsPreserveClaimOnRevert() public {
        ...
        usdg.configure(false, true, address(0));
        vm.prank(alice); vm.expectRevert(TurretRecoverableStaking.UnsupportedTransfer.selector); staking.claim();
    ...
    function testRejectedRecoveryRollsBackAndDoesNotChangeEarnedRewards() public {
        vm.prank(alice); staking.stake(100e18); staking.distributeSubsidy(5e6);
        vm.warp(block.timestamp + 1 days);
        uint256 owed = staking.earned(alice); uint256 remaining = staking.recoverableSubsidy();
        usdg.configure(true, false, address(0));
        vm.expectRevert("token transfer blocked"); staking.recoverSubsidy(remaining);
        assertEq(staking.totalRecovered(), 0); assertEq(staking.earned(alice), owed);
        assertEq(staking.recoverableSubsidy(), remaining);
        usdg.configure(false, true, address(0));
        vm.expectRevert(TurretRecoverableStaking.UnsupportedTransfer.selector); staking.recoverSubsidy(remaining);
        assertEq(staking.totalRecovered(), 0); assertEq(staking.earned(alice), owed);
        usdg.configure(false, false, address(staking));
        staking.recoverSubsidy(remaining); assertTrue(usdg.reentryBlocked());

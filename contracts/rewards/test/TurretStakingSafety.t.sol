...
contract AdversarialStakingToken is ERC20 {
    bool public blocked;
    bool public taxed;
...
contract TurretStakingSafetyTest is Test {
    AdversarialStakingToken token;
    AdversarialStakingToken usdg;
    TurretStaking staking;
    address alice = address(0xA11CE);
    function setUp() public {
        token = new AdversarialStakingToken(); usdg = new AdversarialStakingToken();
    ...
    function testBlockedUSDGClaimDoesNotPreventTURRETWithdrawal() public {
        ...
        vm.prank(alice); staking.claim(); assertEq(usdg.balanceOf(alice), 5e6);
    }
    function testSenderSurchargeCannotDebitMoreTURRETThanReviewed() public {
        token.setSenderTax(true);
        vm.prank(alice); vm.expectRevert(TurretStaking.UnsupportedTransfer.selector); staking.stake(10e18);
        assertEq(token.balanceOf(alice), 100e18);
    ...
    function testTaxedStakingTransferCannotCreateUnbackedStake() public {
        ...
        vm.prank(alice); vm.expectRevert(TurretStaking.UnsupportedTransfer.selector); staking.stake(100e18);
    ...
    function testSenderSurchargeCannotConsumeExtraRewardFunding() public {
        ...
        vm.expectRevert(TurretStaking.UnsupportedTransfer.selector); staking.distribute(5e6);
    ...
    function testTaxedOutgoingRewardsPreserveClaimOnRevert() public {
        ...
        usdg.configure(false, true, address(0));
        vm.prank(alice); vm.expectRevert(TurretStaking.UnsupportedTransfer.selector); staking.claim();
        assertEq(staking.earned(alice), 5e6); assertEq(staking.totalClaimed(), 0);
    }

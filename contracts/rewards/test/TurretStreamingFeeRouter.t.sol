...
contract StreamingSameBlockFeeCapture {
    function attempt(StreamingRouterTokenFixture token, TurretStreamingStaking staking, TurretStreamingFeeRouter router, address pool) external {
...
contract TurretStreamingFeeRouterTest is Test {
    ...
    function setUp() public {
        ...
        router = new TurretStreamingFeeRouter(token, usdg, treasury, pools);
    ...
    function testStreamingSameBlockFeeCaptureRevertsStakeCollectionAndClaimTogether() public {
        ...
        vm.expectRevert(TurretStreamingStaking.WaitOneBlock.selector);
    ...
    function testUnknownPoolCannotPullTreasuryFunds() public {
        ...
        vm.expectRevert(TurretStreamingFeeRouter.UnsupportedPool.selector); router.collect(address(this));
    ...
    function testRepeatedCollectionCannotSpendTreasuryBalanceTwice() public {
        vm.prank(treasury); usdg.approve(address(router), type(uint256).max);
        router.collect(address(pool));
        vm.expectRevert(TurretStreamingFeeRouter.NoFees.selector); router.collect(address(pool));
        assertEq(usdg.balanceOf(treasury), 5e6);
        assertEq(staking.earned(alice), 0);
        assertEq(staking.totalFunded(), 5e6);
    }
    ...
    function testChangedPoolRuntimeRejected() public {
        ...
        vm.expectRevert(TurretStreamingFeeRouter.UnsupportedPool.selector); router.collect(address(pool));
    }
    ...
    function testExternalCallerCannotForwardArbitraryTreasuryFunds() public {
        usdg.mint(treasury, 100e6);
        vm.prank(treasury); usdg.approve(address(router), type(uint256).max);
        vm.expectRevert(TurretStreamingFeeRouter.Unauthorized.selector); router.forwardClaimedFees(100e6);
        assertEq(usdg.balanceOf(treasury), 100e6);
    }
    ...
    function testSubsidyIsTreasuryOnlyAndNotCountedAsBorrowerFees() public {
        ...
        vm.expectRevert(TurretStreamingFeeRouter.Unauthorized.selector); router.fundReserve(1_000e6);

...
contract RecoverableSequenceHandler is Test {
    StakingTokenFixture public token;
    StakingTokenFixture public usdg;
    TurretRecoverableStaking public staking;
    ...
    function checkpointReference() public {
        uint256 released = (staking.totalFunded() - staking.totalRecovered()) * 1e27 - staking.reserveRay();
    ...
    function subsidy(uint64 amount) external { checkpointReference(); staking.distributeSubsidy(bound(amount, 1, 100e6)); }
    function recover(uint64 amount) external { checkpointReference(); uint256 max = staking.recoverableSubsidy(); if (max != 0) staking.recoverSubsidy(bound(amount, 1, max)); }
...
contract TurretRecoverableStakingInvariantTest is StdInvariant, Test {
    RecoverableSequenceHandler handler;
    function setUp() public {
        handler = new RecoverableSequenceHandler();
        ...
        selectors[6] = handler.subsidy.selector; selectors[7] = handler.recover.selector;
    ...
    function invariantStakeAndClaimsMatchIndependentEntitlementLedger() public view {
        TurretRecoverableStaking s = handler.staking(); uint256 supply; uint256 earnedSum;
        for (uint256 i; i < 4; ++i) {
        ...
        assertLe(s.totalReleased() + s.totalRecovered(), s.totalFunded());
        assertLe(s.subsidyReserveRay(), s.reserveRay());
        assertLe(s.totalRecovered(), s.totalSubsidies());
        assertEq(handler.usdg().balanceOf(address(700)), s.totalRecovered());
        assertEq(handler.usdg().balanceOf(address(s)) + s.totalClaimed() + s.totalRecovered(), s.totalFunded() + handler.donations());
    }
}

...
contract TurretRecoverableStakingTest is Test {
    ...
    function setUp() public {
        ...
        staking = new TurretRecoverableStaking(token, usdg, address(this), address(700));
        for (uint256 i; i < 2; ++i) {
    ...
    function testFrequentClaimsDoNotChangeEmissionOrLoseFractionalRewards() public {
        seed();
        TurretRecoverableStaking untouched = new TurretRecoverableStaking(token, usdg, address(this), address(700));
    ...
    function testSameTimestampExitCannotEarnFundingAndDelayStillApplies() public {
        seed(); vm.prank(alice); vm.expectRevert(TurretRecoverableStaking.WaitOneBlock.selector); staking.unstake(100e18);
        vm.roll(block.number + 1); vm.prank(alice); staking.unstake(100e18);
        assertEq(staking.earned(alice), 0);
    ...
    function testTreasuryCanRecoverAllSubsidyWhileEmpty() public {
        staking.distributeSubsidy(1_000e6);
        vm.warp(block.timestamp + 365 days);
        assertEq(staking.recoverableSubsidy(), 1_000e6);
        staking.recoverSubsidy(1_000e6);
        assertEq(usdg.balanceOf(address(700)), 1_000e6);
        assertEq(staking.totalRecovered(), 1_000e6);
        assertEq(staking.reserveRay(), 0);
        assertEq(staking.totalReleased(), 0);
    }
    function testRecoveryCannotTakeEarnedRewardsOrFeeReserve() public {
        seed(); // 1,000 USDG non-recoverable fees.
        staking.distributeSubsidy(1_000e6);
        vm.warp(block.timestamp + 1 days);
        uint256 owed = staking.earned(alice);
        uint256 available = staking.recoverableSubsidy();
        assertApproxEqAbs(available, 990e6, 1);
        vm.expectRevert(TurretRecoverableStaking.InvalidAmount.selector);
        staking.recoverSubsidy(available + 1);
        staking.recoverSubsidy(available);
        assertEq(staking.earned(alice), owed);
        assertEq(staking.recoverableSubsidy(), 0);
        assertGe(staking.reserveRay(), 990e6 * 1e27 - 1e27);
        vm.prank(alice); staking.claim();
        assertEq(usdg.balanceOf(alice), owed);
        vm.warp(block.timestamp + 1 days);
        assertApproxEqAbs(staking.earned(alice), 9_900_000, 2);
    }
    function testRecoveryDoesNotTouchPreviouslyClaimedOrWithdrawnEntitlements() public {
        vm.prank(alice); staking.stake(100e18);
        staking.distributeSubsidy(1_000e6);
        vm.warp(block.timestamp + 1 days); vm.prank(alice); staking.claim();
        uint256 paid = usdg.balanceOf(alice);
        vm.warp(block.timestamp + 1 days); vm.roll(block.number + 1);
        vm.prank(alice); staking.unstake(100e18);
        uint256 owed = staking.earned(alice);
        staking.recoverSubsidy(staking.recoverableSubsidy());
        assertEq(staking.earned(alice), owed);
        vm.prank(alice); staking.claim();
        assertEq(usdg.balanceOf(alice), owed + paid);
        assertLe(staking.totalClaimed() + staking.totalRecovered(), staking.totalFunded());
    }
    function testUnauthorizedRecoveryAndDonationSweepAreRejected() public {
        seed(); usdg.transfer(address(staking), 100e6);
        assertEq(staking.recoverableSubsidy(), 0);
        vm.prank(alice); vm.expectRevert(TurretRecoverableStaking.Unauthorized.selector); staking.recoverSubsidy(1);
        vm.prank(address(700)); vm.expectRevert(TurretRecoverableStaking.Unauthorized.selector); staking.recoverSubsidy(1);
        vm.expectRevert(TurretRecoverableStaking.InvalidAmount.selector); staking.recoverSubsidy(1);
        assertEq(staking.totalRecovered(), 0);
    }
    function testPartialRecoveryAndTopUpDoNotRetroactivelyChangeRewards() public {
        vm.prank(alice); staking.stake(100e18);
        staking.distributeSubsidy(1_000e6);
        vm.warp(block.timestamp + 1 days);
        uint256 owed = staking.earned(alice);
        staking.recoverSubsidy(400e6);
        assertEq(staking.earned(alice), owed);
        staking.distributeSubsidy(1_000e6);
        assertEq(staking.earned(alice), owed);
        vm.warp(block.timestamp + 1 days);
        assertApproxEqAbs(staking.earned(alice) - owed, 15_900_000, 1);
        assertApproxEqAbs(staking.recoverableSubsidy(), 1_574_100_000, 1);
    }
    function testFuzzRecoveryAlwaysLeavesAllEarnedRewardsBacked(uint32 elapsed, uint64 requested) public {
        seed(); staking.distributeSubsidy(1_000e6);
        vm.warp(block.timestamp + bound(elapsed, 1, 10 * 365 days));
        uint256 owed = staking.earned(alice); uint256 available = staking.recoverableSubsidy();
        if (available != 0) staking.recoverSubsidy(bound(requested, 1, available));
        assertEq(staking.earned(alice), owed);
        assertGe(usdg.balanceOf(address(staking)), owed);
        vm.prank(alice); staking.claim(); assertEq(usdg.balanceOf(alice), owed);
        assertLe(staking.totalRecovered() + staking.totalClaimed(), staking.totalFunded());
    }
}

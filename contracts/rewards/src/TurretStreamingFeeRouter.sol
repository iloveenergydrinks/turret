...
import {TurretStreamingStaking} from "./TurretStreamingStaking.sol";
...
contract TurretStreamingFeeRouter is ReentrancyGuard {
    using SafeERC20 for IERC20;
    uint256 public constant STAKER_SHARE_BPS = 5000;
    IERC20 public immutable rewardToken;
    address public immutable treasury;
    TurretStreamingStaking public immutable staking;
    ...
    function _forward(uint256 fee) private returns (uint256 share) {
        ...
        if (share != 0) _fund(share);
    }

    function _fund(uint256 share) private {
        uint256 beforeTreasury = rewardToken.balanceOf(treasury);

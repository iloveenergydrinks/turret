...
import {TurretRecoverableStaking} from "./TurretRecoverableStaking.sol";
...
/// @notice Fund a streaming USDG reserve with half of allowed pools' treasury receipts.
...
contract TurretRecoverableFeeRouter is ReentrancyGuard {
    using SafeERC20 for IERC20;
    uint256 public constant STAKER_SHARE_BPS = 5000;
    IERC20 public immutable rewardToken;
    address public immutable treasury;
    TurretRecoverableStaking public immutable staking;
    ...
    event ReserveSubsidized(uint256 amount);
    event ReserveRecovered(uint256 amount);
    ...
    constructor(IERC20 token, IERC20 usdg, address revenueTreasury, address[] memory allowedPools) {
        ...
        staking = new TurretRecoverableStaking(token, usdg, address(this), revenueTreasury);
    ...
    /// @notice Treasury-only funded subsidy, recorded separately from measured or reported fees.
    /// The exact amount enters the reserve. Protocol fee collection remains 50/50.
    function fundReserve(uint256 amount) external nonReentrant {
        if (msg.sender != treasury) revert Unauthorized();
        if (amount == 0) revert NoFees();
        totalSubsidies += amount;
        _fund(amount, true);
        emit ReserveSubsidized(amount);
    }

    /// @notice Return an exact amount of unearned subsidy to treasury. Fee rewards are protected.
    function recoverReserve(uint256 amount) external nonReentrant {
        if (msg.sender != treasury) revert Unauthorized();
        staking.recoverSubsidy(amount);
        totalRecovered += amount;
        emit ReserveRecovered(amount);
    }

    /// @notice Recover the currently available whole-base-unit subsidy without a stale-amount race.
    function recoverAllSubsidy() external nonReentrant returns (uint256 amount) {
        if (msg.sender != treasury) revert Unauthorized();
        amount = staking.recoverableSubsidy();
        if (amount == 0) revert NoFees();
        staking.recoverSubsidy(amount);
        totalRecovered += amount;
        emit ReserveRecovered(amount);
    ...
    function _forward(uint256 fee) private returns (uint256 share) {
        ...
        if (share != 0) _fund(share, false);
    }

    function _fund(uint256 share, bool subsidy) private {
        uint256 beforeTreasury = rewardToken.balanceOf(treasury);
        ...
        if (subsidy) staking.distributeSubsidy(share);
        else staking.distribute(share);

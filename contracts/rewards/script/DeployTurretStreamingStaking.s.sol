...
contract DeployTurretStreamingStaking is Script {
    function run() external returns (TurretStreamingFeeRouter router) {
        ...
        router = new TurretStreamingFeeRouter(IERC20Metadata(token), IERC20Metadata(usdg), treasury, pools);
        vm.stopBroadcast();

...
contract RealTokenV2ForkTest is Test {
    ...
    function setUp() public {
        ...
        assertEq(assets.length, 12);
        assertEq(holders.length, 12);
        assertEq(amounts.length, 12);
        lender = makeAddr("V2 real-token lender");
        borrower = makeAddr("V2 real-token borrower");
        otherBorrower = makeAddr("V2 competing borrower");
    }
    ...
    function _create(TurretP2PLendingV2 market, uint256 amount) private returns (uint256 id) {
    ...
    function _accept(TurretP2PLendingV2 market, IERC20 collateral, uint256 id, uint256 amount) private {
        ...
        (, address boundBorrower,,,,,, uint256 dueAt, TurretP2PLendingV2.Status status) = market.offers(id);
        assertEq(boundBorrower, borrower);
        assertEq(dueAt, block.timestamp + 31 days);
        assertEq(uint256(status), uint256(TurretP2PLendingV2.Status.Active));
    ...
    function _exerciseAsset(uint256 index) private {
        ...
        TurretP2PLendingV2 market = new TurretP2PLendingV2(cash, collateral, address(this));
        ...
        vm.prank(lender); market.withdraw(cash, PRINCIPAL, lender);
        ...
        market.withdraw(collateral, amount, borrower);
        vm.stopPrank();
        vm.prank(lender); market.withdraw(cash, PRINCIPAL + INTEREST, lender);
        ...
        vm.prank(lender); market.withdraw(collateral, amount, lender);

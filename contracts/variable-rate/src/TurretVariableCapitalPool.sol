...
/// @notice Isolated ERC-4626 ledger with a checkpointed, immutable utilization curve.
...
contract TurretVariableCapitalPool is ERC4626, ReentrancyGuard {
    ...
    uint256 public constant DEBT_LIMIT_DELAY = 1 days;
    uint256 public pendingDebtLimit;
    uint256 public debtLimitValidAfter;
    uint16 public immutable revenueFeeBps;
    // Version 1: principal / (principal + spendable cash); uncollected interest
    // is not cash. Parameters cannot be changed by governance after deployment.
    uint256 public constant rateModelVersion = 1;
    uint16 public immutable baseAprBps;
    uint16 public immutable kinkAprBps;
    uint16 public immutable maxAprBps;
    uint16 public immutable kinkUtilizationBps;
    uint16 public borrowAprBps;
    uint256 private storedRateIntegral;
    ...
    bool public retired;
    bool public liquidationSettlementActive;
    ...
    event BorrowRateChanged(uint16 previousAprBps, uint16 nextAprBps, uint256 utilizationBps);
    event DebtLimitScheduled(uint256 currentLimit, uint256 proposedLimit, uint256 validAfter);
    event DebtLimitChanged(uint256 previousLimit, uint256 newLimit);
    event DebtLimitCancelled(uint256 proposedLimit);
    ...
    constructor(
        ...
        uint16 kinkBps,
        uint16 ceilingBps,
        uint16 utilizationKinkBps
    ) ERC20("Turret variable lender share", "tvLS") ERC4626(IERC20(address(usdg))) {
        if (
            address(usdg).code.length == 0 || usdg.decimals() != 6 || collateral.code.length == 0
                ...
                || treasury == address(this) || limit == 0 || feeBps > 2000 || baseBps > kinkBps || kinkBps > ceilingBps
                || ceilingBps > 10000 || utilizationKinkBps == 0 || utilizationKinkBps >= 10000
        ...
        baseAprBps = baseBps;
        kinkAprBps = kinkBps;
        maxAprBps = ceilingBps;
        kinkUtilizationBps = utilizationKinkBps;
        borrowAprBps = baseBps;
    ...
    /// @notice Only the credit engine's current owner can propose a higher cap.
    /// Existing loans keep their terms; changing this value never moves funds.
    function scheduleDebtLimit(uint256 limit) external onlyEngine {
        if (retired) revert MarketRetired();
        if (limit <= debtLimit) revert InvalidDebtLimit();
        pendingDebtLimit = limit;
        debtLimitValidAfter = block.timestamp + DEBT_LIMIT_DELAY;
        emit DebtLimitScheduled(debtLimit, limit, debtLimitValidAfter);
    }

    function executeDebtLimit() external onlyEngine {
        if (retired) revert MarketRetired();
        uint256 limit = pendingDebtLimit;
        if (debtLimitValidAfter == 0 || block.timestamp < debtLimitValidAfter || limit <= debtLimit) {
            revert DebtLimitNotReady();
        }
        pendingDebtLimit = 0;
        debtLimitValidAfter = 0;
        emit DebtLimitChanged(debtLimit, limit);
        debtLimit = limit;
    }

    /// @notice An immediate reduction may fall below existing principal, including
    /// zero. It blocks additional draws, without forcing repayment or liquidation.
    function lowerDebtLimit(uint256 limit) external onlyEngine {
        if (limit >= debtLimit) revert InvalidDebtLimit();
        _cancelDebtLimit();
        emit DebtLimitChanged(debtLimit, limit);
        debtLimit = limit;
    ...
    function cancelDebtLimit() external onlyEngine {
        _cancelDebtLimit();
    }

    function _cancelDebtLimit() private {
        if (debtLimitValidAfter != 0) emit DebtLimitCancelled(pendingDebtLimit);
        pendingDebtLimit = 0;
        debtLimitValidAfter = 0;
    }

    /// @notice Permanent stop for new lender capital and draws. Settlement stays
    /// available under the existing liquidity, oracle and loss-accounting rules.
    ...
    /// @notice Freeze share entry/exit while collateral has been released but
    /// liquidation proceeds and any loss have not yet been fully settled.
    function beginLiquidationSettlement() external onlyEngine nonReentrant {
        if (liquidationSettlementActive) revert InvalidSettlementState();
        liquidationSettlementActive = true;
    }

    function endLiquidationSettlement() external onlyEngine nonReentrant {
        if (!liquidationSettlementActive) revert InvalidSettlementState();
        liquidationSettlementActive = false;
    }
    ...
    /// @notice Integral of APR basis points over seconds. Both the aggregate
    /// ledger and each borrower use this exact accumulator, without compounding.
    function currentRateIntegral() public view returns (uint256) {
        return storedRateIntegral + (block.timestamp - lastAccrual) * borrowAprBps;
    }

    function utilizationBps() public view returns (uint256) {
        return _utilization(outstandingPrincipal, availableCash());
    }

    function rateAtUtilization(uint256 utilization) public view returns (uint16) {
        if (utilization > 10000) revert InvalidAmount();
        if (utilization <= kinkUtilizationBps) {
            return uint16(baseAprBps + Math.mulDiv(kinkAprBps - baseAprBps, utilization, kinkUtilizationBps));
        }
        return uint16(kinkAprBps + Math.mulDiv(maxAprBps - kinkAprBps,
            utilization - kinkUtilizationBps, 10000 - kinkUtilizationBps));
    }

    /// @notice Indicative APR immediately after this additional loan; other
    /// transactions can change it before signing or later during the loan.
    function previewBorrowAprBps(uint256 additionalPrincipal) external view returns (uint16) {
        uint256 cash = availableCash();
        if (additionalPrincipal > cash) revert InsufficientCash();
        return rateAtUtilization(_utilization(outstandingPrincipal + additionalPrincipal, cash - additionalPrincipal));
    }

    function _utilization(uint256 principal, uint256 cash) private pure returns (uint256) {
        return principal == 0 ? 0 : Math.mulDiv(principal, 10000, principal + cash);
    }

    // Call only after checkpointing past interest and completing the cash or
    // principal mutation. Direct token donations take effect at the next sync,
    // never retroactively. A same-block deposit/withdraw cannot lower past APR.
    function _refreshRate() private {
        uint256 utilization = utilizationBps();
        uint16 next = rateAtUtilization(utilization);
        if (next != borrowAprBps) {
            emit BorrowRateChanged(borrowAprBps, next, utilization);
            borrowAprBps = next;
        }
    }
    ...
    function draw(address recipient, uint256 amount) external onlyEngine nonReentrant {
        ...
        if (outstandingPrincipal >= debtLimit || amount > debtLimit - outstandingPrincipal) {
    ...
    function repay(uint256 principal, uint256 receivedYield) external onlyEngine nonReentrant {
        ...
        protocolFees += fee;
        if (outstandingPrincipal == 0) interestRemainder = 0;
        _refreshRate();
    ...
    function capitalOperationsAllowed() public view returns (bool) {
        if (liquidationSettlementActive) return false;
    ...
    function _accrue() private {
        ...
        storedRateIntegral = currentRateIntegral();
        lastAccrual = block.timestamp;

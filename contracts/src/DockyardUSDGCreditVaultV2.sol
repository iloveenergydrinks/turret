...
interface IRobinhoodStockTokenV2 {
    function oraclePaused() external view returns (bool);
}

/// @title Dockyard USDG Credit Vault V2
...
/// debt ceiling and an immutable global cap. Existing ABI fields are retained
...
contract DockyardUSDGCreditVaultV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    struct Market {
        IDockyardV2Oracle primaryOracle;
        IDockyardV2Oracle secondaryOracle;
        uint128 debtCeiling;
        uint16 maxLtvBps;
    ...
    constructor(address usdg_, address initialOwner, uint16 originationFeeBps_, uint256 globalDebtCeiling_) {
    ...
    function addMarket(
        ...
        if (
            primaryOracle == secondaryOracle || IDockyardRiskOracle(secondaryOracle).collateral() != collateral
                || IDockyardRiskOracle(secondaryOracle).primaryOracle() != primaryOracle
                || IDockyardRiskOracle(secondaryOracle).maxDeviationBps() != maxOracleDeviationBps
        ) revert InvalidOracle();
        uint8 primaryDecimals = _oracleDecimals(primaryOracle);
        uint8 secondaryDecimals = _oracleDecimals(secondaryOracle);
        Market memory market = Market({
            primaryOracle: IDockyardV2Oracle(primaryOracle),
            secondaryOracle: IDockyardV2Oracle(secondaryOracle),
    ...
    function _validatedPrice(address collateral, Market memory market) internal view returns (uint256 value) {
        ...
        (value,) = IDockyardRiskOracle(address(market.secondaryOracle)).validatedPrice(false);
    ...
    {
        _requireStockTokenLive(collateral);
        (value,) = IDockyardRiskOracle(address(market.secondaryOracle)).validatedPrice(true);
        if (value == 0) revert OracleUnavailable();
    }
    ...
    function _oracleDecimals(address oracle) internal view returns (uint8 decimals_) {
        ...
        try IDockyardV2Oracle(oracle).decimals() returns (uint8 value) {
    ...
    function _requireStockTokenLive(address collateral) internal view {
        try IRobinhoodStockTokenV2(collateral).oraclePaused() returns (bool paused_) {

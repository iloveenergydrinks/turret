...
contract DockyardHeartbeatGuard is EIP712 {
    uint8 public constant decimals = 18;
    uint16 public constant maxDeviationBps = 200;
    // Immutable feed-specific heartbeat; independent borrowing quotes remain limited to 60 seconds.
    ...
    constructor(address collateral_, address primary_, address guardian_, uint256 heartbeatSeconds_) EIP712("DockyardChainlinkGuard", "1") {
        if (heartbeatSeconds_ < 60 || heartbeatSeconds_ > 1 days) revert InvalidConfiguration();
        MAX_PRICE_AGE = heartbeatSeconds_;
    ...
    function currentData() public view returns (uint256 value, uint256 updatedAt, uint80 roundId) {
        // An unchanged proxy runtime does not guarantee unchanged feed precision.
        if (primaryOracle.decimals() != primaryDecimals) revert InvalidPrice();

...
contract DeployDockyardHeartbeatPilot is Script {
    string config;
    string marketsJson;
    address guardian;
    function run() external returns(DockyardUSDGCreditVaultPilot vault) {
        require(block.chainid==4663,"Robinhood only");
        config=vm.readFile("./utils/assets/dockyard-pilot-heartbeat-config.json");
        ...
        vm.serializeString("pilot","revision","heartbeat-v2");
        ...
        vm.writeJson(json,"./utils/assets/test_output/dockyard-pilot-heartbeat-candidate.json");
    ...
    function _market(DockyardUSDGCreditVaultPilot vault,uint256 i) internal {
        ...
        DockyardHeartbeatGuard guard=new DockyardHeartbeatGuard(collateral,primary,guardian,_number(path,"heartbeatSeconds",86400));
        ...
        vm.serializeUint(name,"maxPriceAgeSeconds",guard.MAX_PRICE_AGE());

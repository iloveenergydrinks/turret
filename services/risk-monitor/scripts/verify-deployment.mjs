...
const m=readJson('contracts/utils/assets/test_output/dockyard-pilot-candidate.json'),accounts=readJson('output/pilot-deployment/accounts.json');
const broadcast=readJson('contracts/broadcast/DeployDockyardPilot.s.sol/4663/run-latest.json');
const c=createPublicClient({transport:http(process.env.ALCHEMY_RPC_URL,{timeout:12000,retryCount:1})});
assert.equal(await c.getChainId(),4663);assert.equal(m.kind,'chainlink-guarded-pilot');assert.equal(m.guardian.toLowerCase(),accounts.guardian.toLowerCase());
...
for(let start=0;start<23;start+=4){
 receipts.push(...await Promise.all(broadcast.transactions.slice(start,start+4).map(async(p,j)=>{
  ...
  assert.equal(r.status,'success');assert.equal(tx.chainId,4663);assert.equal(tx.from.toLowerCase(),accounts.deployer.toLowerCase());assert.equal(tx.nonce,start+j);assert.equal(tx.value,0n);
...
for(const x of m.markets){
 ...
 assert.equal(age,300n);assert.equal(ttl,60n);assert.equal(delay,120n);assert.equal(deviation,200);
...
writeFileSync(new URL('contracts/utils/assets/test_output/dockyard-pilot-deployed.json',root),JSON.stringify(m,null,2)+'\n');
const json=JSON.stringify(report,(_,v)=>typeof v==='bigint'?v.toString():v,2);writeFileSync(new URL('docs/security/evidence/2026-09-02/pilot-deployment-verification.json',root),json+'\n');

...
async function run() {
  ...
  try {
    ...
    const campaignABI = artifact(rewards, 'TurretPublicBorrowerCashback.sol', 'TurretPublicBorrowerCashback');
    ...
    const policy = { startsAt: start, endsAt: start + 10 * day, settlementDeadline: start + 40 * day,
      claimDeadline: start + 70 * day, rebateBps: 5000, walletCap: '25000000', engines: { [engine.toLowerCase()]: { aprBps: 1000 } } };
    const campaign = await deploy(campaignABI, [usdg, operator.address, operator.address,
      start, policy.endsAt, policy.settlementDeadline, policy.claimDeadline, [engine]]);
    ...
    const config = {chainId:4663,rewardToken:usdg,deployment:{address:campaign,publicEnrollment:true,runtimeHash:keccak256(await client.getCode({address:campaign}))}};
    ...
    console.log('Public enrollment fixture ready: 127.0.0.1:4291 (disposable local chain 4663).');

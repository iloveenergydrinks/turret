...
export const EnvSchema = v.pipe(
  v.object({
    ...
    KNOWN_INITIATIVES_URL: v.optional(v.union([v.pipe(v.string(), v.url()), v.literal("")])),
    ...
    LIQUITY_STATS_URL: v.optional(v.union([v.pipe(v.string(), v.url()), v.literal("")])),
    ...
    ...vBranchEnvVars(8).entries,
    ...vBranchEnvVars(9).entries,
  }),
  v.transform((data) => {
    const env = { ...data };

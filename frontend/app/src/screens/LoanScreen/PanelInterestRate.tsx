...
export function PanelInterestRate({
  ...
  const debt = useInputFieldValue((value) => `${fmtnum(value, "full")} rUSD`, {
  ...
  return (
    <>
        ...
        <UpdateBox
          updates={[
            ...
            },
            {
              label: (
                <>
                  <div>rUSD interest per year</div>
                  <InfoTooltip {...infoTooltipProps(content.generalInfotooltips.interestRateBoldPerYear)} />
                </>
              ),
              before: <Amount value={boldInterestPerYearPrev} suffix=" rUSD" />,
              after: <Amount value={boldInterestPerYear} suffix=" rUSD" />,
            },
          ]}
        />
      </div>
      ...
      {isZombieTrove && (
        <WarningBox>
          <div>
            Interest rate can't be adjusted on loans with debt below 2,000 rUSD. Please adjust your debt first.
          </div>
        </WarningBox>
      )}

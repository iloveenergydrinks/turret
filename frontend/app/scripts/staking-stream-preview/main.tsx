...
const d:StakingDeployment={rewardModel:'reserve-decay-v2',address:'0x3333333333333333333333333333333333333333',router:'0x4444444444444444444444444444444444444444',runtimeHash:hash,routerRuntimeHash:hash,tokenRuntimeHash:hash,usdgRuntimeHash:hash,startBlock:'1'};
const sample:StakingState={account:'0x1111111111111111111111111111111111111111',blockNumber:10n,blockHash:hash,timestamp:1n,canUnstake:true,lastStakeBlock:1n,walletBalance:25000n*10n**18n,staked:10000n*10n**18n,totalStaked:500000n*10n**18n,earned:12500000n,totalFunded:625000000n,totalClaimed:0n,totalCollected:1250000000n,allowance:0n,treasuryAllowance:1000000n};
const scenario=new URLSearchParams(location.search).get('state')??'active';
function Preview(){
 const [earned,setEarned]=useState(sample.earned);useEffect(()=>{const timer=setInterval(()=>setEarned(e=>e+579n),5000);return()=>clearInterval(timer)},[]);
 const [mode,setMode]=useState<'stake'|'unstake'>(scenario==='legacy'?'unstake':'stake'),[amount,setAmount]=useState(''),[review,setReview]=useState<Review>(),[notice,setNotice]=useState('');
